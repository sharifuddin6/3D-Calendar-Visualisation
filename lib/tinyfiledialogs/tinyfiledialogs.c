...
A single C file (add it to your project)
with 6 modal function calls for all platforms:
	...
	char const * const aDefaultInput ) /* "" , if NULL it's a passwordBox */
{
	...
	{
		sprintf(lDialogString, "%s\\AppData\\Local\\Temp\\tinyfdInputBox.hta",
			getenv("USERPROFILE"));
	}
...
Set objFile = objFSO.CreateTextFile(strHomeFolder & \"\\AppData\\Local\\Temp\\tinyfdInputBox.txt\", True)\n\
If result = 1 Then\n\
objFile.Write 0 & txt_input.Value\n\
Else\n\
objFile.Write 1\n\
End If\n\
objFile.Close\n\
...
cscript.exe %USERPROFILE%\\AppData\\Local\\Temp\\tinyfdInputBox.vbs\"");
	...
	{
		strcpy(lDialogString, "powershell -WindowStyle Hidden -Command \"\
mshta.exe %USERPROFILE%\\AppData\\Local\\Temp\\tinyfdInputBox.hta\"");
	}
...
*/
	...
	{
		sprintf(lDialogString, "%s\\AppData\\Local\\Temp\\tinyfdInputBox.txt",
			getenv("USERPROFILE"));
